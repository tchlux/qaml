...
if __name__ == "__main__":
    ...
    TEST_TRUTH_TABLE_GENERATION = False
    if TEST_TRUTH_TABLE_GENERATION:
        # Test the generation of a truth table for addition.
        print('-'*70)    
        # t = int_add_table(n_bits=2, wrap=False)
        t = int_add_table(n_bits=2, signed=False)
        print()
    ...
    if TEST_FIND_QUBO_FOR_TT:
        ...
        for row in tt: print(row)
        print()
        q = find_qubo(tt, random=False, ancilla=False, qubo=QUBO(
            {"a1":1}
        ))
        out = run_qubo(**q)#, min_only=False)
        correct_solution = tuple(tuple(r) for r in tt) == tuple(tuple(r) for r in out)
